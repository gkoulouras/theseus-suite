...
shinyjs.togglemolart = function() {
  ...
  if (x.style.display === "none") {
    ...
    var structBtn2 = document.getElementById("showStructBtn");
	  structBtn2.setAttribute("class", "btn btn-primary btn-sm");
...
shinyjs.seque = function(unipID, seque, geneName, protName, startPos, endPos, identifiable) {
  ...
  for (var i=0; i < startPosArr.length; i++) {
    seqCoverage = seqCoverage.concat("{start:",startPosArr[i]-1,",end:",endPosArr[i]-1,",color:",JSON.stringify(identifiableColArr[i]),", underscore: false}," );
  }
  
  //highlight cleavage sites
 for (var j=0; j < endPosArr.length; j++) {
   if (j !=  endPosArr.length - 1) { //highlight cleavage sites
     seqCoverage = seqCoverage.concat("{start:",endPosArr[j]-1,",end:",endPosArr[j],",color:",JSON.stringify(identifiableColArr[j]),",underscore: false, bgcolor: highlightColor}," );
   }
   else { //remove highlightColor in the last amino acid
     seqCoverage = seqCoverage.concat("{start:",endPosArr[j]-1,",end:",endPosArr[j],",color:",JSON.stringify(identifiableColArr[j]),",underscore: false}," );
   }
  } 
